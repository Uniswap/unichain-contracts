...
This contract is responsible for notifying service contracts about events that happen in the staking middleware. An operator can mint an ERC-721 token representing their operator status. To start operating, the operator can deposit the token into a service contract they would like to operate for. Whenever their voting power changes, through staking, unstaking, delegation or undelegation of delegators as well as slashing, the current owner of the token (service contract) is notified. The service contract can then implement arbitrary logic on these events required for the service contract to function.

To ensure a malicious operator cannot block delegators from unstaking, the notifier cannot revert undelegations from a delegator. In case a malicious operator is blocking a delegator from staking or unstaking, the delegator can forcefully undelegate from the operator and continue staking or unstaking.

To ensure a malicious service contract cannot block an operator from withdrawing their ERC-721 token, the operator can forcefully withdraw their ERC-721 token to themselves. This will notify the service contract so it can implement arbitrary logic on the withdrawal.

The notifier contract utilizes the following hooks:

- `_afterStake`: After a delegator stakes, notify the service contract about the new balances of the operator and the delegator
- `_afterUnstake`: After a delegator unstakes, notify the service contract about the new balances of the operator and the delegator
- `_afterOperatorSelection`: After a delegator delegates to an operator, notify the service contract about the new balances of the operator and the delegator
- `_afterOperatorUndelegationAnnouncement`: After a delegator announces their intention to undelegate from an operator, notify the service contract about the new balances of the operator and the delegator
- `_afterSlash`: After an operator is slashed, notify the service contract about the remaining percentage of the operator's voting power and the remaining percentage of the delegator's stake
