...
contract UniStakerWrapper {
    ...
    function _depositIntoUniStaker(uint256 amount, address delegatee) internal returns (uint256 depositId) {
        ...
        if (depositId == 0) {
            depositId = IUniStaker.DepositIdentifier.unwrap(unistaker.stake(uint96(amount), delegatee));
            ...
            if (amount != 0) {
                unistaker.stakeMore(IUniStaker.DepositIdentifier.wrap(depositId), uint96(amount));
            }
