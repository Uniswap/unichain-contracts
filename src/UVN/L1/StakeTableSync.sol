...
contract StakeTableSync is IStakeTableSync, ERC165 {
    ...
    function reportOperatorSlash(address operator, uint256 remainingPercentage) external onlyStakingMiddleware {
        _depositTransaction(
            abi.encodeWithSelector(IBaseService.reportOperatorSlash.selector, operator, remainingPercentage),
            DEFAULT_GAS_LIMIT
        );
    }
    ...
    function onWithdrawal(address operator) external onlyStakingMiddleware {
        _depositTransaction(abi.encodeWithSelector(IBaseService.onWithdrawal.selector, operator), DEFAULT_GAS_LIMIT);
    }
    ...
    function syncOperator(address operator) public {
        _requireDeposited(operator);
        _syncOperator(operator, DEFAULT_GAS_LIMIT);
    }
    ...
    function onERC721Received(address, address, uint256 tokenId, bytes calldata) external returns (bytes4) {
        ...
        _syncOperator(operator, DEPLOY_GAS_LIMIT);
        ...
        internal
    {
        ...
        // @audit consider edge cases with high gas costs on L2
        _reportOperatorStake(operator, newBalance, delegator, newDelegatorStake, DEFAULT_GAS_LIMIT);
    }
    ...
    function _requireDeposited(address operator) internal view {
        if (IERC721(address(STAKING_MIDDLEWARE)).ownerOf(OperatorTokenLib.toTokenId(operator)) != address(this)) {
            revert OperatorNotDeposited();
        }
    }
}
